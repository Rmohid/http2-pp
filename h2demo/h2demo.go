...
<body>
<h1>RUA + HTTP/2</h1>
<p>Welcome to the RUA project's fork of <a href="https://golang.org/">the Go language</a>'s <a href="https://http2.github.io/">HTTP/2</a> 
demo & interop <a href="https://http2.golang.org">server</a>.</p>
...
<p>Welcome to the RUA project's fork of <a href="https://golang.org/">the Go language</a>'s <a
href="https://http2.github.io/">HTTP/2</a> demo & interop <a href="https://http2.golang.org">server</a>.</p>

<p>Congratulations, <b>you're using HTTP/2 right now</b>.</p>

<p>This server exists for others in the HTTP/2 community to test their HTTP/2 client implementations and point out flaws in our server.</p>

<p> The original code is currently at <a
...
<p> The source for this fork is available in <a href="https://stash.akamai.com/users/rmohid/repos/h2demo/browse">stash</a>
 A depandancy-free binary is available on request</p>
...
<ul>
  ...
  <li>GET <a href="/gophertiles">/gophertiles</a> to see a page of server pushed tiled images</li>
  <li>GET <a href="/simple_show">simple_show</a> to display a given set of css+js+png resources</li>
  <li>GET <a href="/simple_push">simple_push</a> server push resource set and provide links</li>
  <li>GET <a href="/simple_prefetch">simple_prefetch</a> send prefetch headers+links for resource set</li>
...
<p>A large assortment of test handlers is also available on the <a href="https://nghttp2.org/httpbin/">nghttp interop server</a> </p>
...
func simpleShowHandler(w http.ResponseWriter, r *http.Request) {
	fmt.Fprintf(w, "<html><head>\n")
	for k, _ := range resourceList {
		if strings.HasPrefix(k, "push.js.") {
			fmt.Fprintf(w, "<script type=\"text/javascript\" src=\"/file/%s\"></script>", k)
		}
		if strings.HasPrefix(k, "push.css.") {
			fmt.Fprintf(w, "<link type=\"text/css\" rel=\"stylesheet\" href=\"/file/%s\">", k)
		}
	}
	// To prove the js loaded
	fmt.Fprintf(w, "<script> $(document).ready(function(){ $(\".pic\" ).fadeOut(\"slow\",function(){}).fadeIn(\"slow\",function(){});}); </script>")
	fmt.Fprintf(w, "</head><body><div>\n")
	for k, _ := range resourceList {
		if strings.HasPrefix(k, "push.img.") {
			fmt.Fprintf(w, "<img class=\"pic\" src=\"/file/%s\"><br>\n", k)
		}
	}
...
func simplePrefetchHandler(w http.ResponseWriter, r *http.Request) {
	// See http://www.w3.org/TR/resource-hints/
	for k, _ := range resourceList {
		if strings.HasPrefix(k, "push.js.") {
			w.Header().Add("Link", fmt.Sprintf("<https://%s/file/%s>; rel=prefetch;  as=script", httpsHost(), k))
		}
		if strings.HasPrefix(k, "push.css.") {
			w.Header().Add("Link", fmt.Sprintf("<https://%s/file/%s>; rel=prefetch", httpsHost(), k))
		}
	}
	fmt.Fprintf(w, "<html><body>\n")
	for k, _ := range resourceList {
...
func registerHandlers() {
	...
	resourceList = map[string]string{
		"gopher.png":     "https://golang.org/doc/gopher/frontpage.png",
		"go.src.tar.gz":  "https://storage.googleapis.com/golang/go1.4.1.src.tar.gz",
		"push.img.a.png": "https://golang.org/doc/gopher/doc.png",
		"push.img.b.png": "https://golang.org/doc/gopher/help.png",
		"push.img.c.png": "https://golang.org/doc/gopher/ref.png",
		"push.css.d.css": "https://golang.org/lib/godoc/style.css",
		"push.js.e.js":   "https://ajax.googleapis.com/ajax/libs/jquery/1.11.3/jquery.min.js",
	}

	mux2.HandleFunc("/", home)
	for k, v := range resourceList {
		mux2.Handle("/file/"+k, fileServer(v))
	}
	mux2.HandleFunc("/reqinfo", reqInfoHandler)
	mux2.HandleFunc("/crc32", crcHandler)
	mux2.HandleFunc("/clockstream", clockStreamHandler)
	mux2.HandleFunc("/simple_show", simpleShowHandler)
	mux2.HandleFunc("/simple_push", simplePushHandler)
	mux2.HandleFunc("/simple_prefetch", simplePrefetchHandler)
